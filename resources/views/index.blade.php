...
@section('content')
    ...
    <div class="block sm:hidden space-y-3">
        @forelse ($stagiaires as $stg)
            <a href="{{ route('stagiaires.show', $stg->id) }}"
                ...
                <div class="flex items-center gap-3">
                    <div
                        ...
                        {{ strtoupper(substr($stg->Fullname, 0, 1)) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-medium text-white truncate">{{ $stg->Fullname }}</p>
                        <p class="text-xs text-slate-400 truncate">{{ $stg->email }}</p>
                    </div>
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse ($stagiaires as $stg)
                        <tr class="hover:bg-white/5 transition-colors duration-200 group">
                            <td class="px-6 py-4 text-sm text-slate-400 font-mono">{{ $loop->iteration }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                    ...
                                    <span class="text-sm font-medium text-white">{{ $stg->Fullname }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-300">{{ $stg->email }}</td>
